...
def main():
    ...
    if result and "properties" in result:
        ...
        print(f"   Total precip: {df['PRECTOTCORR'].sum():.1f} mm ({df['precip_inches'].sum():.1f} in)")
        ...
        seasonal_temp = 10 + 15 * np.sin(2 * np.pi * (day_of_year - 100) / 365)  # Base 10°C + 15°C amplitude
